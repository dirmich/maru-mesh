...
Tailscale은 MagicDNS를 통해 device 이름으로 접속할 수 있게 합니다. MaruMesh도 `dev`, `dev.maru`, `<device-id>.maru` 형태의 이름 해석을 목표로 합니다.
...
MaruMesh는 agent가 peer 정보를 받아 로컬 DNS resolver와 hosts file에 반영하는 방식입니다.

차이:

- Tailscale: MagicDNS가 제품 기능으로 통합됨.
- MaruMesh: `dev`, `dev.maru`, `<device-id>.maru`를 로컬에 반영하는 방식이며, hosts file 쓰기 권한이 없으면 제한될 수 있음.
