...
- **주요 구현 완료 사항**:
    ...
    - **정책 초기 동기화 보강**: agent 시작 직후 control plane 정책을 동기화하고, 실패 시 기존 로컬 정책을 유지.
    - **다중 에이전트 테스트**: `test/alice_home`, `test/bob_home` 분리를 통한 동일 기기 내 다중 실행 환경. 테스트 상태·키·로그는 `test/` 아래에 두고 Git에서 제외.
    ...
    - **Client 설치 스크립트**: `install.sh`/`install.ps1`은 public repo root에 동기화해 GitHub raw URL로 제공하고, public 배포 repo `dirmich/maru-mesh`의 GitHub Releases에서 client 바이너리를 받아 설치하며 `CONTROL_URL` 환경변수를 반영.
    - **실행파일 자체 서비스 설치**: `marumesh install-service`로 Linux systemd, macOS LaunchAgent, Windows Service 설치 가능.
    - **CLI version/upgrade**: `marumesh version`으로 현재 binary 버전을 출력하고, `marumesh upgrade`로 public repo installer를 실행해 최신 release로 갱신.
    - **Mac App Store Connect 패키징**: `make appstore-pkg`와 `scripts/build_macos_appstore_pkg.sh`로 Apple Distribution 서명 `.app`과 3rd Party Mac Developer Installer `.pkg`를 생성. 현재는 업로드 가능한 패키지 형식이며, App Store 심사 통과를 위해서는 sandbox/Network Extension 기반 앱 구조 보강이 필요.
    - **Tray/Menu icon 보강**: macOS menubar template PNG와 Windows tray ICO를 분리하고, 주요 menu item에도 icon을 설정.
    - **Device 재등록 안정화**: 같은 `device_id`는 중복 row를 만들지 않고, SSO 재로그인/재등록 시 기존 device 이름을 덮어쓰지 않으며 owner별 DNS 이름 충돌을 차단.
    ...
    - **Client install URL**: 공식 설치 진입점은 public repo raw `install.sh`/`install.ps1`이며, 스크립트가 GitHub Releases의 client 바이너리를 설치하도록 정리. 공식 테스트 URL은 `https://marumesh.lab.highmaru.com`.
    ...
    - **device 관리 보강**: 동일 device 재등록은 기존 row의 이름을 보존하고 public key/최종 로그인 시간을 갱신하며, owner/superuser가 이름을 수정 가능.
    - **Virtual IP 대역 설정**: 신규 device 기본 가상 IP 대역을 `100.64.0.0/24`로 변경하고 `MARUMESH_VIRTUAL_CIDR`로 운영 환경에 맞게 변경 가능.
    - **Peer 이름 해석**: peer hostname을 로컬 DNS/hosts에 반영해 `dev`, `dev.maru`, `<device-id>.maru` 이름 접근을 지원.
